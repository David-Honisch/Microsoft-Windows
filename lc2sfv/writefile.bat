...
set "filename=%output_dir%\file_%timestamp%.sfv"
set outFile=lc2crcdirs.csv
REM dir /ad>appdirs.csv
set exeFile=%~dp0lc2sfv.exe
REM dir /b /s *.exe>%outFile%
call %exeFile% --directory %%1 --output %filename%
REM for /f "usebackq tokens=1-5 delims=;" %%a in ("%outFile%") do (
	REM echo call %exeFile% %%~na%%~xa %%a %%~na%%~xa.lnk
	REM echo call %exeFile% --directory %%a --output %filename%
	REM call %exeFile% --directory %%a --output %filename%

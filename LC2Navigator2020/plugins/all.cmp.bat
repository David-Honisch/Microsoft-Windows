...
type nul > %target%
echo SELECT '^<p^>Count of plugins:'||count(*)||'^</p^>' to plugins; >> %target%
...
echo SELECT '^<p^>Count of all imported plugins:'||count(*)||'^</p^>' to plugins; >> %target%
echo SELECT '^<h3^>Running SCRIPT DONE^</h3^>'; >> %target%

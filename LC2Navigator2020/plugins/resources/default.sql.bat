...
echo INSERT OR REPLACE INTO application(name, first_name, description, zipcode, city, street, url)VALUES('%in% Plugin v.1.02a','%in% Plugin v.1.01a','','','','','execCMD(''exec .\\resources\\plugins\\%in%\\index.bat .\\resources\\plugins\\%in%\\menu.csv'', ''out'');');  >> %out%

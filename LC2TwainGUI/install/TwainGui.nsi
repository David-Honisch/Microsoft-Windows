...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "TwainGui"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "Comments" "TwainGui Client"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "TwainGui"
...
Name "TwainGui"
Caption "TwainGui Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "TwainGuiinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\TwainGui"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\TwainGui" "TwainGui"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users TwainGui.lnk" $INSTDIR\TwainGui.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\TwainGui "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\TwainGui" "DisplayName" "TwainGui (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\TwainGui" "UninstallString" '"$INSTDIR\uninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\TwainGui.exe"
  ;File /a "exe\*.*"
  File /a ".\install.zip"
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\TwainGui "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\TwainGui "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\TwainGui "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\TwainGui "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\TwainGui "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\Start.LC2Launcher.lnk" $INSTDIR\TwainGui.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\TwainGui.lnk" $INSTDIR\TwainGui.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\_LC2Launcher.lnk" $INSTDIR\TwainGui.exe
 CreateShortCut "$INSTDIR\Update.lnk" $INSTDIR\LC2Start.NET.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\AutoStart.lnk" $INSTDIR\LC2autostart.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\AutoStartConfig.lnk" $INSTDIR\LC2autostartConfig.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\AutoStartUninstall.lnk" $INSTDIR\LC2autostartUninstall.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\LC2Config.lnk" $INSTDIR\LC2Config.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\Uninstall.lnk" $INSTDIR\uninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
UninstallText "Uninstall TwainGui ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\TwainGui"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\TwainGui"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\TwainGui\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  ...
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\TwainGui.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\uninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\TwainGui\*.*"

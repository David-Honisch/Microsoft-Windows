...
window.view = require(path.join(webRoot,'radioview.js'));
...
$('document').ready(function () {
  window.model.getQuery('SELECT * FROM `radio` ORDER BY `name` ASC');
  $('#searchsubmit').click(function (e) {
    e.preventDefault();
    window.model.getQuery('SELECT * FROM `radio` WHERE name like "%'+$('#search').val()+'%" ORDER BY `name` ASC');
  })
  $('#edit-person-submit').click(function (e) {
    ...
    if (ok) {
      ...
      window.model.saveFormData('radio', keyValue, function () {
        window.model.getQuery('SELECT * FROM `radio` ORDER BY `name` ASC');
      })
    }
  })

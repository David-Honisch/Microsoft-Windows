...
window.view = require(path.join(webRoot,'ebooksview.js'));
...
$('document').ready(function () {
  window.model.getQuery('SELECT * FROM `ebooks` ORDER BY `name` ASC');
  $('#searchsubmit').click(function (e) {
    e.preventDefault();
    window.model.getQuery('SELECT * FROM `ebooks` WHERE name like "%'+$('#search').val()+'%" ORDER BY `name` ASC');
  })
  $('#edit-person-submit').click(function (e) {
    ...
    if (ok) {
      ...
      window.model.saveFormData('ebooks', keyValue, function () {
        window.model.getQuery('SELECT * FROM `ebooks` ORDER BY `name` ASC');
      })
    }
  })

...
window.view = require(path.join(webRoot,'gamesview.js'));
...
$('document').ready(function () {
  window.model.getQuery('SELECT * FROM `games` ORDER BY `name` ASC');
  $('#searchsubmit').click(function (e) {
    e.preventDefault();
    window.model.getQuery('SELECT * FROM `games` WHERE name like "%'+$('#search').val()+'%" ORDER BY `name` ASC');
  })
  $('#edit-person-submit').click(function (e) {
    ...
    if (ok) {
      ...
      window.model.saveFormData('games', keyValue, function () {
        window.model.getQuery('SELECT * FROM `games` ORDER BY `name` ASC');
      })
    }
  })

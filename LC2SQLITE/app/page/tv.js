...
window.view = require(path.join(webRoot,'tvview.js'));
...
$('document').ready(function () {
  window.model.getQuery('SELECT * FROM `tv` ORDER BY `name` ASC');
  $('#searchsubmit').click(function (e) {
    e.preventDefault();
    window.model.getQuery('SELECT * FROM `tv` WHERE name like "%'+$('#search').val()+'%" ORDER BY `name` ASC');
  })
  $('#edit-person-submit').click(function (e) {
    ...
    if (ok) {
      ...
      window.model.saveFormData('tv', keyValue, function () {
        window.model.getQuery('SELECT * FROM `tv` ORDER BY `name` ASC');
      })
    }
  })

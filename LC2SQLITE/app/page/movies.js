...
window.view = require(path.join(webRoot,'moviesview.js'));
...
$('document').ready(function () {
  window.model.getQuery('SELECT * FROM `movies` ORDER BY `name` ASC');
  $('#searchsubmit').click(function (e) {
    e.preventDefault();
    window.model.getQuery('SELECT * FROM `movies` WHERE name like "%'+$('#search').val()+'%" ORDER BY `name` ASC');
  })
  $('#edit-person-submit').click(function (e) {
    ...
    if (ok) {
      ...
      window.model.saveFormData('movies', keyValue, function () {
        window.model.getQuery('SELECT * FROM `movies` ORDER BY `name` ASC');
      })
    }
  })

...
window.view = require(path.join(webRoot,'musicview.js'));
...
$('document').ready(function () {
  window.model.getQuery('SELECT * FROM `music` ORDER BY `name` ASC');
  $('#searchsubmit').click(function (e) {
    e.preventDefault();
    window.model.getQuery('SELECT * FROM `music` WHERE name like "%'+$('#search').val()+'%" ORDER BY `name` ASC');
  })
  $('#edit-person-submit').click(function (e) {
    ...
    if (ok) {
      ...
      window.model.saveFormData('music', keyValue, function () {
        window.model.getQuery('SELECT * FROM `music` ORDER BY `name` ASC');
      })
    }
  })

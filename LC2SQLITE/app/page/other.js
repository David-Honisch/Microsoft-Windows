...
window.view = require(path.join(webRoot,'otherview.js'));
...
$('document').ready(function () {
  window.model.getQuery('SELECT * FROM `other` ORDER BY `name` ASC');
  $('#searchsubmit').click(function (e) {
    e.preventDefault();
    window.model.getQuery('SELECT * FROM `other` WHERE name like "%'+$('#search').val()+'%" ORDER BY `name` ASC');
  })
  $('#edit-person-submit').click(function (e) {
    ...
    if (ok) {
      ...
      window.model.saveFormData('other', keyValue, function () {
        window.model.getQuery('SELECT * FROM `other` ORDER BY `name` ASC');
      })
    }
  })

...
module.exports.showPeople = function (rowsObject) {
  ...
  for (let rowId in rowsObject) {
    ...
    let execUrl = "new HttpRequest().execCMD('"+row.url+"', 'out')"; 
    ...
    '<div class="col-xs-5 name"><a href="' +openlink+ row.url + '" target="_blank">' + row.url + '</a></div>' +
    '<div class="col-xs-5 name"><a href="#" onclick="' + execUrl + '" target="_parent">more...</a></div>' +
    '</div>';
  }
...
module.exports.editPerson = function (pid) {
  ...
  pid = pid.split('_')[1];
  let row = model.getPerson(pid, 'SELECT * FROM `people` WHERE `person_id` IS ?')[0];

...
async fn main() {
	...
	let app = "LC2DL";
	let version = 1.0;
	let author = "David Honisch";
	println!("{:?} Version {:?} (c) by {:?}", app, version, author);

	let args: Vec<String> = env::args().collect();
	let url = if args.len() > 1 { &args[1] } else { "" };
	let localFile = if args.len() > 2 { &args[2] } else { "" };
	...
	download_file(&Client::new(), url, localFile).await.unwrap();

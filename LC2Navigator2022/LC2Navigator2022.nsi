...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2Navigator2022"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "Comments" "LC2Navigator2022 Client x64"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2Navigator2022 x64"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileVersion" "1.0.0"

;--------------------------------


Name "LC2Navigator2022"
Caption "LC2Navigator2022 Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2Navigator2022install.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2Navigator2022"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2Navigator2022" "LC2Navigator2022"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2Navigator2022.lnk" $INSTDIR\LC2Navigator2022.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2Navigator2022 "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Navigator2022" "DisplayName" "LC2Navigator2022 (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Navigator2022" "UninstallString" '"$INSTDIR\uninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
  ;File /a "exe\LC2Navigator2022.exe"
  ;File /a "exe\*.*"
  File /a ".\install.zip"
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Navigator2022 "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Navigator2022 "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Navigator2022 "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Navigator2022 "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Navigator2022 "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateDirectory $SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022
...
; CreateShortCut "$SMPROGRAMS\Startup\lc2search.exe.lnk" "$INSTDIR\lc2search-win32-ia32\lc2search.exe"
 ...
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\AutoStart.lnk" $INSTDIR\LC2autostart.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\AutoStartConfig.lnk" $INSTDIR\LC2autostartConfig.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\AutoStartUninstall.lnk" $INSTDIR\LC2autostartUninstall.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\LC2Config.lnk" $INSTDIR\LC2Config.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\Uninstall.lnk" $INSTDIR\uninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
UninstallText "Uninstall LC2Navigator2022 ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Navigator2022"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2Navigator2022"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2Navigator2022\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  ...
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\LC2Navigator2022.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\uninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Navigator2022\*.*"

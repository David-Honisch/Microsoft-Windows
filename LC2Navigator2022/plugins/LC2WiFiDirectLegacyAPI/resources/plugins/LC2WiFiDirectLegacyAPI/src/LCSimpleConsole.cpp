...
#include "stdafx.h"
#include "LCSimpleConsole.h"
#include "WlanHostedNetworkWinRT.h"

LCSimpleConsole::LCSimpleConsole()
...
void LCSimpleConsole::RunConsole()
{
    std::wstring command;
    bool running = true;
...
void LCSimpleConsole::OnDeviceConnected(std::wstring remoteHostName)
{
    std::wcout << std::endl << "Peer connected: " << remoteHostName << std::endl;
}

void LCSimpleConsole::OnDeviceDisconnected(std::wstring deviceId)
{
    std::wcout << std::endl << "Peer disconnected: " << deviceId << std::endl;
}

void LCSimpleConsole::OnAdvertisementStarted()
{
...
void LCSimpleConsole::OnAdvertisementStopped(std::wstring message)
{
    std::wcout << "Soft AP stopped." << std::endl;
    SetEvent(_apEvent.Get());
}

void LCSimpleConsole::OnAdvertisementAborted(std::wstring message)
{
    std::wcout << "Soft AP aborted: " << message << std::endl;
    SetEvent(_apEvent.Get());
}

void LCSimpleConsole::OnAsyncException(std::wstring message)
{
    std::wcout << std::endl << "Caught exception in asynchronous method: " << message << std::endl;
}

void LCSimpleConsole::LogMessage(std::wstring message)
{
    std::wcout << std::endl << message << std::endl;
}

bool LCSimpleConsole::AcceptIncommingConnection()
{
...
void LCSimpleConsole::ShowPrompt()
{
    std::wcout << std::endl << ">";
}
...
bool LCSimpleConsole::ExecuteCommand(std::wstring command)
{
